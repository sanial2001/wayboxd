...
**Coding standards** are in `.cursor/rules/`. **Workflows** are in `.agents/skills/` (including `new-email-template`, `model-layer-types`). **Domain context** is in `docs/agent/domain-map.md`.
...
- Copy `.env.example` to `.env.local`. Photon search requires `PHOTON_BASE_URL` and `PHOTON_USER_AGENT`.
